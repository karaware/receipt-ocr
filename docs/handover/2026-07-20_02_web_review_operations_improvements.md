...
受け入れ確認の丸亀製麺レシートでは、明細が次の構成だった。

```text
かけ(大)             630円  食費 / 外食
かしわ天              220円  食費 / 外食
鮭おむすび            170円  食費 / 外食
5枚天ぷら100円引     -100円  調整 / 値引き・税
合計                  920円
```

このまま値引きを `調整` にすると、食費（外食）のカテゴリ集計は `1,020円` となり、実際に外食へ支払った `920円` と一致しない。全体の支出額だけは920円になるが、カテゴリ別の食費集計が100円過大になる。

値引きがレシート内の食費購入に対するものと分かる場合は、負の明細を **`食費 / 外食`、金額 `-100円`** として登録する。これにより食費（外食）の集計も920円になる。

`調整 / 値引き・税` は、複数カテゴリにまたがり配賦先を決められない値引き、またはカテゴリに属さない端数・税額の調整に限定する。

将来の改善候補として、OCR/LLMの出力または確認画面で「値引きの適用先カテゴリ」を指定できるようにする。単一カテゴリのレシートでは、既存の他明細と同じカテゴリを初期値にするのが自然である。
