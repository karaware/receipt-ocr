...
cloud-init完了後、ローカルから一時的に `opc` のホームへ転送し、rootで所定位置へinstallする。

```bash
scp config/config.json opc@VM_PUBLIC_IP:/home/opc/config.json
scp deploy/oci/config.example.env opc@VM_PUBLIC_IP:/home/opc/config.env
scp secrets/drive.json secrets/vision.json secrets/firestore.json opc@VM_PUBLIC_IP:/home/opc/

ssh opc@VM_PUBLIC_IP
sudo install -o root -g receipt-ocr -m 0640 /home/opc/config.json /etc/receipt-ocr-poc/config.json
sudo install -o root -g receipt-ocr -m 0640 /home/opc/config.env /etc/receipt-ocr-poc/config.env
sudo install -o receipt-ocr -g receipt-ocr -m 0600 /home/opc/drive.json /etc/receipt-ocr-poc/secrets/drive.json
sudo install -o receipt-ocr -g receipt-ocr -m 0600 /home/opc/vision.json /etc/receipt-ocr-poc/secrets/vision.json
sudo install -o receipt-ocr -g receipt-ocr -m 0600 /home/opc/firestore.json /etc/receipt-ocr-poc/secrets/firestore.json
rm /home/opc/config.json /home/opc/config.env /home/opc/drive.json /home/opc/vision.json /home/opc/firestore.json
```

`config.env` のプレースホルダーを実値へ変更してから転送する。`config.json` は本番Mac用設定をそのまま使わず、
PoC専用設定を用意する。
...
```bash
sudo -u receipt-ocr /opt/receipt-ocr/.venv/bin/python \
  -m receipt_ocr --config /etc/receipt-ocr-poc/config.json cloud-worker --poc --dry-run

sudo -u receipt-ocr /opt/receipt-ocr/.venv/bin/python \
  -m receipt_ocr --config /etc/receipt-ocr-poc/config.json cloud-worker --poc --once

sudo systemctl enable --now receipt-ocr-poc.timer
systemctl list-timers receipt-ocr-poc.timer
journalctl -u receipt-ocr-poc.service -n 100 --no-pager
```
...
```bash
python -m receipt_ocr --config /etc/receipt-ocr-poc/config.json cloud-worker --poc --dry-run
python -m receipt_ocr --config /etc/receipt-ocr-poc/config.json cloud-worker --poc --once
python -m receipt_ocr --config /etc/receipt-ocr-poc/config.json cloud-worker status --poc
python -m receipt_ocr --config /etc/receipt-ocr-poc/config.json cloud-worker retry DRIVE_FILE_ID --poc
```
