...
OCR結果をFirebaseへ同期し、本人・妻が外出先から使える家計簿Webアプリを
`web-app/` に実装しています。

主な機能:

- Googleログインと世帯単位の許可リスト
- 月次ダッシュボード、取引入力、レシート確認、予算、カテゴリ管理
- Firestoreへの冪等なOCR同期と失敗時の再送
- 説明可能な値引き・税・10円以下の端数調整
- Webで修正した商品分類ルールのMac側への反映

Firebaseの仕組みは `docs/FIREBASE_GUIDE.md`、初期設定とデプロイ方法は
`docs/FIREBASE_SETUP.md` を参照してください。
